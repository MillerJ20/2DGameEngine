...
public:
  ...
  void Update(SDL_Renderer* renderer,
    ...
    for (auto entity : GetSystemEntities()) {
      ...
      if (health.healthPercentage >= 30 && health.healthPercentage < 60) {
        // 60-30 = orange
        healthBarColor = {255, 172, 28};
      }
      if (health.healthPercentage >= 60 && health.healthPercentage < 90) {
        // 40-80 = yellow
      ...
      // Position the health bar indicator in the top-right part of the entity
      // sprite
      int healthBarWidth = 15;
      int healthBarHeight = 3;
      double healthBarPosX =
          (transform.position.x + (sprite.width * transform.scale.x)) -
          camera.x;
      double healthBarPosY = (transform.position.y) - camera.y + 10;

      SDL_Rect healthBarRectangle = {
          static_cast<int>(healthBarPosX), static_cast<int>(healthBarPosY),
          static_cast<int>(healthBarWidth * (health.healthPercentage / 100.0)),
          static_cast<int>(healthBarHeight)};
      SDL_SetRenderDrawColor(renderer, healthBarColor.r, healthBarColor.g,
      ...
      SDL_Surface* surface =
          TTF_RenderText_Blended(assetStore->GetFont("charriot-font"),
                                 healthText.c_str(), healthBarColor);
