...
public:
  ...
  void SubscribeToEvents(const std::unique_ptr<EventManager>& eventBus) {
    eventBus->SubscribeToEvent<CollisionEvent>(this,
                                               &MovementSystem::OnCollision);
  }

  void OnCollision(CollisionEvent& event) {
    Entity a = event.a;
    Entity b = event.b;
    Logger::Log("Collision Event between enemy and obstacle!");
    if (a.BelongsToGroup("enemies") && b.BelongsToGroup("obstacles")) {
      Logger::Log("If statement passed, a is enemy!");
      OnEnemyHitsObstacle(a, b); // "a" is the enemy, "b" is the obstacle
    }
    if (a.BelongsToGroup("obstacles") && b.BelongsToGroup("enemies")) {
      Logger::Log("If statement passed, b is enemy!");
  ...
  void Update(double deltaTime) {
    ...
    for (auto entity : GetSystemEntities()) {
      ...
      // Check if entity is outside the map boundaries (with a 200 pixels
      // forgiving culling margin)
