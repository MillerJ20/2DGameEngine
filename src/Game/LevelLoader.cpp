...
void LevelLoader::LoadLevel(sol::state& lua,
  ...
  lua.script_file("./assets/scripts/Level" + std::to_string(levelNumber) +
                  ".lua");
  ...
  while (true) {
    ...
    if (assetType == "texture") {
      ...
      Logger::Log("A new texture asset was added to the asset store, id: " +
                  assetId);
    }
    if (assetType == "font") {
      assetStore->AddFont(assetId, asset["file"], asset["font_size"]);
      Logger::Log("A new font asset was added to the asset store, id: " +
                  assetId);
    }
    i++;
  }
  ...
  for (int y = 0; y < mapNumRows; y++) {
    for (int x = 0; x < mapNumCols; x++) {
      ...
      tile.AddComponent<SpriteComponent>(mapTextureAssetId, tileSize, tileSize,
                                         0, false, srcRectX, srcRectY);
    }
  }
  mapFile.close();
      ...
      if (projectileEmitter != sol::nullopt) {
        newEntity.AddComponent<ProjectileEmitterComponent>(
            ...
            static_cast<int>(entity["components"]["projectile_emitter"]
                ...
                1000,
            static_cast<int>(entity["components"]["projectile_emitter"]
                                   ["hit_percentage_damage"]
                                       .get_or(10)),
            entity["components"]["projectile_emitter"]["friendly"].get_or(
                false));
      }
      ...
      if (keyboardControlled != sol::nullopt) {
        newEntity.AddComponent<KeyboardControlledComponent>(
            glm::vec2(
                ...
                entity["components"]["keyboard_controller"]["up_velocity"]
                      ["y"]),
